...
//		$store['bank'] = $bank;
    ...
    {
        $agent = D('Yws_agent_applyView');

        $condition = array();
        if (!empty($_GET['type']) && !empty($_GET['keyword'])) {
            if ($_GET['type'] == 'store_id') {
                $condition['Yws_agent_apply.store_id'] = trim($_GET['keyword']);
            }
            else if ($_GET['type'] == 'uid') {
                $condition['Yws_agent_apply.uid'] = trim($_GET['keyword']);
            }
            else if ($_GET['type'] == 'id') {
                $condition['Yws_agent_apply.id'] = trim($_GET['keyword']);
            }
            else if ($_GET['type'] == 'name') {
                $condition['Yws_agent_apply.name'] = array('like', '%' . trim($_GET['keyword']) . '%');
            }
            else if ($_GET['type'] == 'tel') {
                $condition['Yws_agent_apply.tel'] = array('like', '%' . trim($_GET['keyword']));
            }
        }
        //if (!empty($_GET['agent_type'])) $condition['_string'] = "(Agent.agent_id = '{$_GET['agent_id']}')";

        if (isset($_GET['status']) && is_numeric($_GET['status']))  $condition['status'] = $_GET['status'];


        $agent_count = $agent->where($condition)->count('Yws_agent_apply.id');
        //dump($agent_count);
        import('@.ORG.system_page');
        $page = new Page($agent_count, 10);
        $agents = $agent->where($condition)
            ->order('Yws_agent_apply.id DESC')
