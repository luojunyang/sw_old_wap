...
class UserAction extends BaseAction
{
	...
	public function index()
	{
		...
		//==========================================test
            ...
            /*$sql = "select u.nickname, u.openid, u.uid,u.last_time, a.uid,a.address,a.name,a.tel from tp_user_test u left join (select `uid`,`name`,`tel`,`address` from tp_user_address group by `uid`) as a on u.uid = a.uid where u.`openid` <> '' and a.tel <> ''";
            //$sql = "select tmp.uid from (select u.nickname, u.openid, u.uid,u.last_time, a.uid,a.address,a.name,a.tel from tp_user u left join (select `uid`,`name`,`tel`,`address` from tp_user_address group by `uid`) as a on u.uid = a.uid where u.`openid` <> '' and a.tel <> '') as tmp";
            //$sql = "select u.nickname, u.openid, u.uid, a.uid,a.address,u.last_time,a.name,a.tel from tp_user u left join tp_user_address as a on u.uid = a.uid where u.`openid` <> '' and a.tel <> ''";
            //$sql = "select openid,phone from tp_user  where `openid` <> '' and  phone  <> ''";
            //$sql = "select openid,last_time from tp_user  where `openid` <> '' and  last_time  <> ''";
            //$sql = "select `uid`,`name`,`tel`,`address` from tp_user_address group by `uid`";
            $d= D();
            $user = $d->query($sql);
            ...
            $d=D('User_test')->where(array('uid'=>array('not in ',$uids)))->delete();
            dump($d);*/
        //==================================================
