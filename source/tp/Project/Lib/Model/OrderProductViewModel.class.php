...
class OrderProductViewModel extends ViewModel
{
    public $viewFields = array(
        'OrderProduct' => array('*'),
        'Product_test' => array('product_id', 'name', 'image', '_on' => 'OrderProduct.product_id = Product_test.product_id'),
    );

    public function getProducts($order_id)
    {
        return $this->field(array("Product_test.product_id AS product_id", "Product_test.name AS name", "pro_num", "pro_price", "sku_data", "comment", "is_packaged", "in_package_status", "image"))->where(array('OrderProduct.order_id' => $order_id))->select();
    }
