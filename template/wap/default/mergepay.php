				...
				<div class="app-inner inner-order" id="js-page-content">
					...
					<div class="block block-order block-border-top-none">
						...
						<div class="block block-list block-border-top-none block-border-bottom-none">
							<?php $total_no_psoatge=0;/*<div class="block-item name-card name-card-3col clearfix">
							...
							foreach ($nowMerge['orders'] as $value) { ?>
								...
								<div class="bottom" style="line-height: 1em;">
									总价<span class="pull-right">￥
                                        <span class="c-orange"><?php echo $value['sub_total']; ?></span>
                                        + ￥<span class="c-orange"><?php echo $value['postage']; ?></span>
                                        = ￥<span class="c-orange"><?php echo $value['total']; ?></span>
                                    </span>
                                    <?php $total_no_psoatge += $value['sub_total']?>
                                </div>
								<?php
								if($value['status'] == 0) { ?>
									<div class="order-message clearfix" id="js-order-message">
										<textarea class="js-msg-container font-size-12" data-id="<?php echo $item['order_id']; ?>" placeholder="给卖家留言..."></textarea>
									</div>
								<?php }
								else { ?>
...
<!--							<input type="hidden" name="address_id" id="address_id" value="--><?php //echo intval($userAddress['address_id']); ?><!--" />-->
	...
	<script type="text/javascript">
		$(function () {
			...
			var balance_canbeuse =  parseFloat('<?php echo round($total_no_psoatge * 0.1, 2); ?>');
            function balance_input_value()
            {
                console.log(balance,balance_canbeuse);
                if(balance){
                    console.log(balance,balance_canbeuse);
                    if(balance >= balance_canbeuse){
                        $('#balance').val(balance_canbeuse);
                    }else{
                        $('#balance').val(balance);
                    }
                }
