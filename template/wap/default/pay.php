						...
						<div class="block block-list block-border-top-none block-border-bottom-none">
							<?php if(isset($_GET['orderName'])) { ?>
								<div class="block-item name-card name-card-3col clearfix">
									<div class="detail" style="margin-left:0px;width: 200px;">
                    ...
                    <?php if($show_flag):?>
						...
						<?php if($nowOrder['status'] < 1) {?>
							<div class="js-logistics-content logistics-content js-express">
								<?php if($userAddresses && ($now_store['open_logistics'] || $is_all_supplierproduct)) {?>
										<div>
                                            <div class="block block-form block-border-top-none block-border-bottom-none">
												<?php foreach ($userAddresses as $uAddr){?>
								...
								<?php } /*else {*/ ?>
									<div class="js-order-address express-panel">
										<?php if($now_store['open_logistics'] || $is_all_supplierproduct) { ?>
...
<!--								</div>-->
                    ...
                    <div class="block express" id="js-logistics-container">
                        <div class="block-item logistics">
                            <h4 class="block-item-title">联系方式</h4>
                        </div>
                        <div>
                            <table style="margin: 0 0 0 20px;">
                                <tr >
                                    <th>联系人：</th>
                                    <td ><input type="text" name="tour_address_user" value="" style="width: 100%;height: 30px;border:1px solid #ccc;margin: 10px 0 5px 0;"><br/></td>
                                </tr>
                                <tr >
                                    <th>联系电话：</th>
                                    <td ><input type="text" name="tour_address_tel" value="" style="width: 100%;height: 30px;border:1px solid #ccc;margin: 5px 0;" minlength="11" maxlength="11"><br/></td>
                                </tr>
                                <tr >
                                    <th>推荐人手机：</th>
                                    <td ><input type="text" name="tour_share_phone" value="" placeholder="选填" style="width: 100%;height: 30px;border:1px solid #ccc;margin: 5px 0 10px 0;"></td>
                                </tr>
                            </table>
                        </div>
                    </div>
							...
							<div class="js-user_coupon block-item order-total" style="text-align:left;">
								...
								if($wap_user['balance'] > 0) { ?>
                                    ...
                                    <div style="width: 100% ; height: 20px;" >
                                        <p style="color: red;" id="can_use_balance">
                                    </div>
		...
		$(function () {
			...
			var total = parseFloat('<?php
				echo $nowOrder['sub_total'] * 1 + $nowOrder['postage'] * 1 - ($reward_money +
						$user_coupon_money + $nowOrder['balance'] +
						$nowOrder['point'] * 1.00 / $config['point_exchange']); ?>');
			var total_no_postage = parseFloat('<?php
                echo $nowOrder['sub_total'] * 1  - ($reward_money +
                        $user_coupon_money + $nowOrder['balance'] +
                        $nowOrder['point'] * 1.00 / $config['point_exchange']); ?>');
			var balance_can_use = parseFloat(total_no_postage * 0.1).toFixed(2)  ;
			console.log('can_use:'+balance_can_use);
			var rate = parseFloat('<?php echo $config['point_exchange']; ?>');

			function balance_input_value()
            {
                console.log(balance);
                if(balance){
                    if(balance >= balance_can_use){
                        $('#balance').val(balance_can_use);
                    }else{
                        $('#balance').val(balance);
                    }
                }
			...
			$('.txt-ye').change(function () {
				...
				if (b == '' || isNaN(b)) {
					...
					if (b <= 0) {
						...
						//if (b <= b) {
						if (balance_can_use <= b) {
							b = balance_can_use;
                            if (balance_can_use > balance) {
                                b = balance;
                                //$('#can_use_balance').html('注意：能使用余额不足').show().delay(2000).hide(0);
                                //$('#can_use_balance').html('注意：能使用余额不足').slideToggle(2000);
                                $('#can_use_balance').html('注意：能使用余额不足').slideDown(2000).delay(1000).slideUp(1000);
                            }else{
                                $('#can_use_balance').html('注意：使用余额最多只能是付款总数的10%').slideDown(2000).delay(1000).slideUp(1000);
                                $('#point').val(0);
                            }
                            //$('#can_use_balance').html('注意：使用余额最多只能是付款总数的10%').show().delay(2000).hide(0);
                            //$('#can_use_balance').html('注意：使用余额最多只能是付款总数的10%').slideToggle(2000);
