$(function () {
	var checkCartPro = function () {
        console.log($('.block-item-cart').length);
		if ($('.block-item-cart').length == 0) {
			$('.js-list-header').hide();
			...
			$('#cart-container').html('<div class="empty-list" style="margin-top:30px;"><div class="empty-list-header"><h4>购物车要饿瘪了 T.T</h4><span>快给我挑点宝贝</span></div><div class="empty-list-content"><a href="' + storeUrl + '" class="home-page tag tag-big tag-orange">去逛逛</a></div></div>');
	...
	var checkCart = function () {
		...
		//console.log(typeof cartMoney);
        //console.log( cartMoney);
        var cartMoneyint = cartMoney.toFixed(2);
        //console.log(cartMoneyint);
		// $('.js-total-price').html(cartMoney);
		$('.js-total-price').html(cartMoneyint);
		if (cartCount) {
	...
	$('.js-delete-goods').click(function () {
		...
		if (ids.length > 0) {
			...
			$.post('./cart.php?action=del', {storeId: storeId, ids: ids}, function (result) {
				...
				if (result.err_code == 0) {//block-item block-item-cart relative clearfix item-424
					for (var i in ids) {
