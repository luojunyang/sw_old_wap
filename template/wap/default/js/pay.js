...
//		}
	...
	$('#confirm-pay-way-opts .btn-pay').click(function () {
		...
		if(show_flag){
            ...
            var loadingCon = $('<div style="text-align: center; overflow:hidden;visibility:visible;position:absolute;z-index:1100;transition:opacity 300ms ease;-webkit-transition:opacity 300ms ease;opacity:1;top:' + (($(window).height() - 100) / 2) + 'px;left:' + (($(window).width() - 200) / 2) + 'px;"><div class="loader-container" style="text-align:center;width: 200px;background: #fff;padding: 50px 10px;text-align: center;"><div class="loader center">处理中，请稍候...</div></div></div>');
            var loadingBg = $('<div style="text-align: center; height:100%;position:fixed;top:0px;left:0px;right:0px;z-index:1000;opacity:1;transition:opacity 0.2s ease;-webkit-transition:opacity 0.2s ease;background-color:rgba(0,0,0,0.901961);"></div>');
			...
			var address_user = $('input[name=tour_address_user]').val();
			var address_tel = $('input[name=tour_address_tel]').val();
			var share_phone = $('input[name=tour_share_phone]').val();
			if(address_user=='' || address_tel=='' ){
                motify.log('请填写联系人和联系电话');
                return false;
			}
            var payType = $(this).data('pay-type');
