...
var returnCash = function (total_money)
{
    if(total_money>=10000){
        price_back = 500;
    }else if (total_money>=5000){
        price_back = 200;
    }else if (total_money>=2000){
        price_back = 80;
    }else if (total_money>=1000){
        price_back = 60;
    }else if (total_money>=500){
        price_back = 40;
    }else if (total_money>=300){
        price_back = 20;
    }else if (total_money>=200){
        price_back = 10;
            ...
            if (result.err_code) {
                ...
                if (result.err_msg.list.length > 0) {
                    ...
                    for (var i in result.err_msg.list) {
                        ...
                        var price_back = returnCash(product.price);
                        /*productHtml += '<div class="hproduct ' + (productShowRows ? '' : 'item_long_cover') + '">' +
                         ...
                         '<p class="prices">' + (product.market_price == product.price ? '' : '<del><em>¥&nbsp;' + (product.discount <10 ? (product.price/product.discount*10).toFixed(2) : product.market_price) + '</em></del>') +
                         ...
                         '<p class="back" style="margin:0px 0 0 56px">购买立返 <strong><em>' + price_back +'元</em></strong></p>' +
                         '<p class="sku"><span style="margin:-2px 0 0 6px" class="comment_num ' + (product.sales == '0' ? 'hide' : 'hide') + '">销量 <span>' + product.sales + '</span></span>&nbsp;<span class="stock hide">无货</span></p>' +
                         '</a></div>';*/
                        ...
                        productHtml += '<div class="hproduct ' + (productShowRows ? '' : 'item_long_cover') + '">' +
                            ...
                            '<strong><em>¥&nbsp;' + product.price + '</em></strong>'+ discountStr +'</p>' +
                            // '<p class="back" style="margin:0px 0 0 56px">购买立返 <strong><em>' + (product.point == undefined || product.point == 0 ? '￥' + product.rebate : (product.point<0?0:product.point)+'积分') + '</em></strong></p>' +
                            '<p class="back" style="margin:0px ">购买立返 <strong><em>' + price_back +'元</em></strong><span style="" class=" ' + (product.sales == '0' ? 'hide' : '') + '">销量 <span>' + product.sales + '</span></span>&nbsp;<span class="stock hide">无货</span></p>' +
                            '</a></div>';
                    }
