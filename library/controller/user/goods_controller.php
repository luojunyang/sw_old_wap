...
//            foreach ($supplierStoreInfo as $val){
//                if($val['store_id']==91 || $val['store_id']==212){
//                    continue;
//                }
//                $arr[$val['name']] = $val['store_id'];
//            }
            //dump($arr);
            //$where['store_id'] = array('in',$arr);
            //$where['status'] = '1';
            //$where['is_fx'] = '1';
            //$where['quantity'] = array('>', '0');

//            $product = D('Product_test')->select();
//            dump(count($product));
        ...
        if (IS_POST) {
            ...
            if (!empty($product_id)) {
                foreach ($product_id as $id) {
                    $product_info =
                        D('Product_test')->field('product_id,is_fx,fx_type,source_product_id,original_product_id')
    ...
    public function edit_group()
    {
        ...
        if (!empty($data)) {
            foreach ($data as $value) {
                ...
                if ($flag) {
                    D('Product_test')->where(array('product_id' => $product_id))->data(array('has_category' => 1))->save();
                }
            }
            json_return(0, '修改成功');
        } else {
...
//		$product_to_group = M('Product_to_group');
    ...
    public function del_product()
    {
        ...
        if (!empty($product_id)) {
            foreach ($product_id as $id) {
                $product_info = D('Product_test')->field('product_id,is_fx,fx_type,source_product_id,original_product_id')
                    ->where(array('product_id' => $id))->find();
    ...
    public function set_sort()
    {
        if (IS_POST) {
            ...
            if (D('Product_test')->where(array('product_id' => $id, 'store_id' => $this->store_session['store_id']))
    ...
    {
        $products = D('Product_test')->where(array('source_product_id' => $product_id))->select();
        if (!empty($products)) {
            foreach ($products as $product) {
                D('Product_test')->where(array('product_id' => $product['product_id']))->data(array('status' => 2))->save();
    ...
    */
    ...
    private function _comment_goods_list()
    {
        ...
        if ($count > 0) {
            ...
            $produdcts = D('Product_test')->where(array('product_id' => array('in', $in_array)))->select();
