...
class order_product_model extends base_model{
	...
	public function orderProduct($order_id,$getPro=true){
		if($getPro){
			$orderProduct = D('')->table(array('Product_test'=>'p','Order_product'=>'op'))->where("`op`.`order_id`='$order_id' AND `op`.`product_id`=`p`.`product_id`")->order('`op`.`id` ASC')->select();
		}else{
