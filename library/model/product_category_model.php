...
//			}
	...
	public function getAllCategory($limit="", $is_show_son=false) {
		...
		if($is_show_son) {
			foreach($category_list as $k=>$v) {
                ...
                //推荐分类
                if($v['cat_id']==465){
                    $all_recommend = $this->db->where(array('is_recommend'=>1))->select();
                    if(!empty($all_recommend)){
                        foreach ($all_recommend as $k=>$val){
                            if($val['cat_fid'] == 465 || $val['cat_fid']==0){
                                $cat_ids[] = $k;
                            }
                        }
                        ...
                        $levelArray = array_merge($levelArray,$all_recommend);

                        //$categorie = array_shift($levelArray);
                        foreach($levelArray as &$items) {
                            $sort[] = $items['cat_sort'];
                        }
                        array_multisort($sort,$levelArray);
                        //array_unshift($levelArray,$categorie);
                    }
                }
	...
	public function getCategories($wheres =array()) {
		...
		$categories = $this->db->where($wheres)->order('cat_sort ASC')->select();
		return $categories;
	}
