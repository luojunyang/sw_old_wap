...
if ($payType == 'yeepay') {
    ...
    $payInfo = $payClass->notice();
    logs(var_export($payInfo,true),'payInfo');
    pay_notice_call($payInfo);
...
function pay_notice_call($payInfo, $ok_msg = 'success', $err_msg = 'fail')
{
    if ($payInfo['err_code'] === 0) {
        ...
        logs('nowproduct'.var_export($nowOrder,true),'INFO');
...
//				}
                //申请订单
                if($agentflag){
                    $res = D('Yws_agent_apply')->where(array('order_id'=>$order_id))->data(array('status'=>1))->save();
                }else{
                    /**
                     * 分享记录
                     */
                    $share_data = explode('_',$nowOrder['address']);
                    if(count($share_data)==2){
                        $share_phone = empty($share_data[0])?'false':$share_data[0];
                        $share_from = empty($share_data[1])?'false':$share_data[1];
                    }
                    //logs( $nowOrder['address'].'var'.$share_phone.'-'. $share_from.'-'.var_export($share_data,true),'share_test');
                    $d= D('');

                    if($share_phone || $share_from){
                        //查询订单中的商品
                        //分享活动商品
                        $product_ids = "13005，13007";
                        $arr_products = explode(',',$product_ids);

                        $sql = " select op.product_id,op.pro_num,p.product_id from tp_order_product as op LEFT JOIN tp_product_test as p on op.product_id=p.product_id where op.order_id={$order_id}";
                        $product = $d->query($sql);
                        logs(var_export($product,true).'sql'.$sql,'share_product');
                        //查看购买人的联系电话
                        $phone = $nowOrder['address_tel'];
                        //查看被那些人分享
                        $where = "from_uid = {$_SESSION['user']['uid']} OR share_phone = {$phone}";
                        $buy_uids = D('Yws_share')->where($where)->select();
                        $str_ids = '';
                        foreach ($buy_uids as $val){
                            $ids[] = $val['buy_uid'];
                            $phones[] = $val['address_tel'];
                            //$str_ids .= ",'".$val['buy_uid']."'";
                        }

                        //判断是否交叉分享
                        $flag = true;
                        if(in_array($share_from,$ids) || in_array($share_phone,$phones)){
                            $flag = false;
                        };
                        //logs($flag,'shareflag');
                        //插入分享记录
                        foreach ($product as $val){
                            //if($val['product_id']==50870){
                            //if($flag && in_array($val['product_id'],$arr_products)){
                            if($flag){
                                $data['buy_uid'] = $_SESSION['user']['uid'];
                                $data['address_tel'] = $phone;
                                $data['order_id'] = $order_id;
                                $data['product_id'] = $val['product_id'];
                                $data['from_uid'] = $share_from;
                                $data['share_phone'] = $share_phone;
                                $data['count'] = $val['pro_num'];
                                $data['buy_name'] = $nowOrder['address_user'];
                                $data['add_time'] = date('Y-m-d H:i:s',time());
                                //dump($data);
                                $res = D('Yws_share')->data($data)->add();
                                logs($_SESSION['user']['uid'] . ':,sharecount:' .
                                    $_SESSION['share_from'] .'or'.$_SESSION['share_phone'] .',info:'.
                                    var_export($data,true).','.
                                    var_export($ids, true), 'SHAREdata');
                            }
                        }
                    }
                    //logs($_SESSION['share_from'] .'-or-'.$_SESSION['share_phone'],'shareTYPE');

//=============================一元抢购============================================================================================
                    $onlygoods_id = array(13267,13270,13276,13277,13278);
                    //订单中的商品
                    $sql = " select op.product_id,op.pro_num,p.product_id from tp_order_product as op LEFT JOIN tp_product_test as p on op.product_id=p.product_id where op.order_id={$order_id}";
                    $product = $d->query($sql);
                    logs(var_export($product,true).'-sql'.$sql,'onlyone');
                    foreach ($product as $val){
                        if(in_array($val['product_id'],$onlygoods_id)){
                            $res = D('Yws_onlyone')->where(array('order_id'=>$order_id))->data(array('status'=>1))->save();
                            logs($res,'res');
                        }
                    }
                }

                $price_back = returnCash($nowOrder['pay_money']);
                logs($price_back.'paymoney'.$nowOrder['pay_money'],'return_cash');
                D('User')->where(array('uid'=>$nowOrder['uid']))->setInc('balance',$price_back);
