...
sku_quantity, ps.weight sku_weight, ps.price sku_price, ps.cost_price sku_cost_price FROM tp_order_product uc INNER JOIN tp_product_test p ON uc.product_id = p.product_id LEFT JOIN tp_product_sku ps ON uc.sku_id = ps.sku_id WHERE uc.order_id = {$ord['order_id']} ORDER BY uc.id DESC";
