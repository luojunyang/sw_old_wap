...
//logs(var_export($nowOrder['proList'],true),'test');
//dump($_GET['id']);
...
if($nowOrder['openStroePay']==1) redirect('./yws_agent_apply.php');
...
//logs(var_export($nowOrder['proList'],true),'PRO');
//dump($show_flag);
$isImport = M('Product_test')->checkIsImport(implode(',',$productIds));
...
//logs(var_export($nowOrder,true),'test');
// dump($nowOrder);
// 付款方式
