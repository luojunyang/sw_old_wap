...
function pay_notice_call($payInfo, $ok_msg = 'success', $err_msg = 'fail')
{
	if($payInfo['err_code'] === 0) {
        ...
        //购买立返余额
        $price_back =(int) returnCash($nowMerge['pay_money']);

        logs($price_back.':paymoney:'.$nowMerge['pay_money'],'return_cash');

        D('User')->where(array('uid'=>$nowMerge['uid']))->setInc('balance',$price_back);
		logs('mergypaynotice:'.var_export($nowMerge,true),'nowMerge');
