...
//=================================================一元抢购
            ...
            $only_db = D('Yws_onlyone');
            if(in_array($_POST['proId'],$onlygoods_id)){
                for($i=0;$i<$quantity;$i++){
                    $data['number'] = date('YmdHis',time()).mt_rand(1000,9999).$i;
                    $data['order_id'] = $order_id;
                    $data['goods_id'] = $_POST['proId'];
                    $data['uid'] = $wap_user['uid'];
                    $only_id = $only_db->data($data)->add();
                }
            }
//===================================================end
    ...
    case 'pay':
        ...
        if(isset($_POST['show_flag'])){
            ...
            if (empty($nowOrder['status'])) {
                ...
                $data_order['address'] = $_POST['share_phone']."_".$_SESSION['share_from'];
                $_SESSION['share_phone'] = $_POST['share_phone'];
                $data_order['status'] = '1';
...
//				}
            ...
            //一元购商品
            $onlygoods_id = array(13267,13268,13269,13270,13271);
            //=====end
            $model_order = M('Order');
            $db_order = D('Order');
            $nowOrder = $model_order->find($_POST['orderNo']);
...
//				}
    ...
    case 'refund' : //退款
        ...
        if ($nowOrder['status'] < 2) {
            ...
            //2 支付
            //3 改变各种状态
            if($nowOrder['pay_money']>=20){
                $orderforuser = $db_user->where(array('uid'=>$nowOrder['uid']))->find();
                $price_back = returnCash($nowOrder['pay_money']);
                if($orderforuser['balance']>=$price_back){
                    $db_user->where(array('uid'=>$nowOrder['uid']))->setDec('balance',$price_back);
                }
            }
